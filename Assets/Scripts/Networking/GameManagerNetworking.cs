...
public class GameManagerNetworking : MonoBehaviour
{
    ...
    public void SetPlayerTwoClient(GameObject otherPlayer)
    {
        ...
        UIFacade.Singleton.onlineMultiplayer.SetActiveOnlineMultiplayer(false);
        UIFacade.Singleton.localMultiplayer.SetActiveLocalMultiplayer(true);
    ...
    private void SetPlayerWord()
    {
       /*if(!isServer)
        ...
        players[localPlayerIndentity].SetWord(UIFacade.Singleton.localMultiplayer.currentInputFieldText);
    ...
    private void CheckForCharOnRivalPlayerWord()
    {
      /* if(!isServer)
        ...
        Dictionary<int, char> correctChars = 
            players[otherPlayerIndex].CheckForCharsInWord(UIFacade.Singleton.localMultiplayer.currentInputFieldText[0]);
        ...
        if (correctChars.Count == 0)
        {
            ...
            {
                case 0:
                    UIFacade.Singleton.localMultiplayer.playerOneErrors.text = string.Format("Player 1 Errors: {0}/10", players[0].errorsCount);
                    break;

                case 1:
                    UIFacade.Singleton.localMultiplayer.playerTwoErrors.text = string.Format("Player 2 Errors: {0}/10", players[1].errorsCount);
                    break;
